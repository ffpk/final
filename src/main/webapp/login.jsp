	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#captchaImage").click(function(){//点击更换验证码
                $("#captchaImage").prop("src","defaultKaptcha?t="+new Date().getTime());
			});
            ...
            var f1=function(){
                var v1=$("#name").val();
                if(v1.length==6){
                    $("#name").next().html("用户名输入正确");
                    return true;
                }else{
                    $("#name").next().html("用户名输入不正确");
                    return false;
                }
            };
            $("#name").blur(f1);
            var f=function(){
                var v1=$("#pwd").val();
                if(v1.length==6){
                    $("#pwd").next().html("密码输入正确");
                    return true;
                }else{
                    $("#pwd").next().html("密码输入不正确");
                    return false;
                }
            };
				...
				<table>
					<tbody>
					  ...
					  <tr>
							...
							<td>
								<input type="password" id="pwd" name="password" class="text" value="" maxlength="20" autocomplete="off"/>
							</td>
					  </tr>
